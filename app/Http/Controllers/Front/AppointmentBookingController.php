...
//                $admin_user->deposit($amount);
    ...
    public function payTmorder($appointmentID,$mobile_no,$email)
    {
        ...
        $payment->prepare([
          ...
          'callback_url' => 'http://localhost:8001/payTm-callback'
        ]);
    ...
    public function mentorArchievedAppointment(Request $request){
        ...
        $token="123";
        if ($request->token==$token){
            BookAppointment::where([['mentor_id',$request->mentor_id]
            ,['id',$request->appointment_id]])->update(['is_archieve'=>1]);
    ...
    public function mentorUnarchievedAppointment(Request $request){
        ...
        if ($request->token==$token){
            BookAppointment::where([['mentor_id',$request->mentor_id]
            ,['id',$request->appointment_id]])->update(['is_archieve'=>0]);
    ...
    public function menteeArchievedAppointment(Request $request){
        ...
        $token="123";
        if ($request->token==$token){
            BookAppointment::where([['mentee_id',$request->mentee_id]
            ,['id',$request->appointment_id]])->update(['is_archieve'=>1]);
