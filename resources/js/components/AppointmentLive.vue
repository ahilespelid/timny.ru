...
<div class="container">
  <div class="row">
    <div class="col-md-12 ps-4 pt-5" style="padding-bottom:9rem !important" >
        ...
        <h5 class="text-primary mt-5 mt-md-0">Принять или Отклонить запрос на встречу в режиме реального времени</h5>
...
data(){
  ...
  methods: {
    ...
    async sendBookedAppointmentNotification() {
      const params = {
        ...
        body: "Ваш Запрос На Живую Встречу Принят",
        title: "Новая встреча для Вас.",
  ...
  created() {
    console.log(
      "selected_new_date" + this.appointment_id,
      "user_selected_slot" + this.appointment_fee
    );
    this.checkLoggedIn();
    if (this.is_loggedIn && this.User.role == "Mentor") {
    } else if (this.is_loggedIn && this.User.role == "Mentee") {
      this.$toasted.error("Пожалуйста, войдите в систему как консультант");
    } else {
